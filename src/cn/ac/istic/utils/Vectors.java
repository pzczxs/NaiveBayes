...
public class Vectors {
    ...
    public static double[] scale(double[] vec, double lower, double upper) {
    	if (lower >= upper) {
			throw new IllegalArgumentException("lower: " + lower + " must be greater than upper: " + upper); 
		}
    	
    	final double minVal = min(vec); 
    	final double maxVal = max(vec); 
    	...
    	for (int i = 0; i < vec.length; i++) {
    		vec[i] = (vec[i] - minVal) * (upper - lower) / (maxVal - minVal) + lower; 
    	}
